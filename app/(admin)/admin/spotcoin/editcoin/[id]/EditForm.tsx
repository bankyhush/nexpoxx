...
interface AdminCoinEditFormProps {
  id: string | number;
}

export default function AdminCoinEditForm({ id }: AdminCoinEditFormProps) {
  ...
  useEffect(() => {
    const fetchCoin = async () => {
      try {
        const res = await fetch(`/api/admin_api/coins/${id}`);
        ...
        setForm({
          ...
          coinRate: data.coinRate?.toString() || "",
          photo: data.photo || "",
          withMin: data.withMin?.toString() || "",
          withMax: data.withMax?.toString() || "",
          withInstructions: data.withInstructions || "",
          depositInstructions: data.depositInstructions || "",
          depositAddress: data.depositAddress || "",
          percent: data.percent?.toString() || "",
          desc: data.desc || "",
          coinVisible: data.coinVisible ? "true" : "false",
        });
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await toast.promise(
        fetch(`/api/admin_api/coins/${id}/update`, {
          method: "PUT",
          ...
          if (!res.ok) {
            ...
            throw new Error(data.error || "Coin update failed");
          }

          return data;
        }),
        {
          loading: "Updating coin...",
          success: "Coin updated successfully!",
          error: (err) => err.message || "Failed to update coin",
        }
      );
  ...
  return (
    ...
    >
      <h2 className="text-xl font-bold mb-4">Edit Coin</h2>
